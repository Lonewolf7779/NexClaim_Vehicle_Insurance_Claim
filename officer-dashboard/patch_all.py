...
for file_path in target_files:
    ...
    content = content.replace('className=\"glass-panel\"', '')
    # Same for masonry layout wrapper - wait, masonry layout might be structurally needed, but we can remove its class
    content = content.replace(' className=\"dashboard-masonry\"', '')
    ...
    with open(file_path, 'w', encoding='utf-8') as f:
        f.write(content)
    print(f'Done rewriting {file_path}.')
